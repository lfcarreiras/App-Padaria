...
export const ThermalReceipt: React.FC<ThermalReceiptProps> = ({ encomenda, loja, onClose }) => {
  const handlePrint = () => {
    window.print();
  };

  const itensPadaria = encomenda.itens.filter((i) => i.setor === 'padaria');
  const itensPastelaria = encomenda.itens.filter((i) => i.setor === 'pastelaria');
      ...
      <div className="relative flex max-h-[90vh] w-full max-w-md flex-col rounded-2xl bg-white shadow-2xl">
        ...
        <div className="no-print flex items-center justify-between border-b border-gray-200 p-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-gray-900">Talão de Encomenda (80mm)</h3>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
        ...
        {/* ÁREA DE IMPRESSÃO TÉRMICA (80mm) */}
        <div className="overflow-y-auto p-6 bg-gray-50 flex justify-center">
          <div
            id="thermal-print-area"
            className="w-[300px] border border-dashed border-gray-300 bg-white p-4 font-mono text-xs text-black shadow-sm"
          >
            {/* Cabeçalho da Loja */}
            <div className="text-center">
              <p className="font-bold text-sm uppercase">{loja?.nome || encomenda.loja_nome || 'Padaria & Pastelaria'}</p>
              <p className="text-[11px]">{loja?.morada || 'Lisboa, Portugal'}</p>
              <p className="text-[11px]">Tel: {loja?.telefone || '210 000 000'}</p>
              {loja?.nif && <p className="text-[10px]">NIF: {loja.nif}</p>}
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between text-[11px]">
                ...
                <span className="font-bold uppercase">
                  {encomenda.estado_pagamento === 'pago' ? `[ PAGO - ${encomenda.metodo_pagamento || ''} ]` : '[ A COBRAR NO ATO ]'}
                </span>
              </div>
            </div>
